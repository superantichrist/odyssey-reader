...
- 번역 수록 범위: 전 24권, 1,186개 문단 전체(1차 직접 번역 완료)
- 영구 링크: 권과 행 번호를 `?book=1&line=1` 형태로 보존
- 원문·직역·출전 URL 함께 복사
- 읽기 자료: 핵심 용어 20개, 인물 32명, 장소 20곳, 판본·본문비평 주석 7개
...
`scripts/build_data.py`가 고정한 TEI 원문, 별도 번역 파일, `references/`의 읽기 자료를 결합하여 `public/data/` 아래 정적 JSON을 만듭니다. 빌드는 원문 해시, 권·행·문단 수, 행의 중복·누락, 알려진 원전 행 번호 공백과 모든 참고 자료의 본문 링크를 검증합니다.
